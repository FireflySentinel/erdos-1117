...
component. These sets are locally finite, since the singular and
ramification loci are discrete and \(\Phi\) is proper.

For each irreducible component \(\Gamma_i\) of \(\Gamma\), the map
\[
 \Phi^{-1}(\Gamma_i\setminus T)\longrightarrow\Gamma_i\setminus T
\]
is a finite covering. The regular part of an irreducible curve remains
connected after deletion of a locally finite set; see
\cite[Chapter IV, Section 3, Corollary 1]{Herve}. Hence there is an
...
Each irreducible component of \(\X^*\) comes from an irreducible
component of \(\X\), and its image under \(\Phi\) is an irreducible
component of \(\Gamma\). Every component of \(\Gamma\) arises in this
way.
...
Generative AI tools were used substantially during the development of
this work. OpenAI's GPT-6, accessed through Codex, was used to develop
parts of the proof, including the logarithmic-derivative formulation,
the implicit-function and direct-tract argument in
Section~\ref{sec:products}, the finite-map argument in
Section~\ref{sec:finite}, and the sharpness example in
Remark~\ref{rem:sharp}. It was also used for literature search, proof
checking, and language and \LaTeX{} editing. The author reviewed and
verified the mathematical arguments and references and takes full
