...
The proof uses continuation of implicit functions and growth in direct
tracts to study the complex curve
\(A(z)=\overline{A(\overline w)}\), where \(A=zf'/f\).
On the locus \(zw\ne0\) and \(A(z)\notin\{m,\infty\}\), the map
\((z,w)\mapsto(zw,A(z))\) is finite and proper. Counting its fibres
near the coordinate axes gives the bound.
No growth assumption on \(f\) is required.
\end{abstract}
...
To explain the argument, set \(A=zf'/f\). At any radius where
\(\log M(r,f)\) is differentiable with respect to \(\log r\), all
maximum modulus points have the same real value of \(A\).
Thus they give points \((z,\overline z)\) of the complex curve
\(A(z)=\overline{A(\overline w)}\), with the same values of both
\(zw\) and \(A(z)\).
We study the fibres of the resulting map of curves.
The main step is to show that every irreducible component contains
points with \(|zw|\) arbitrarily small. At such points one coordinate
lies near the origin, where \(A-m\) has local degree \(k\).
Properness then carries this local count to every component of the
image, outside a discrete set.

\section{Two analytic facts}\label{sec:facts}

We use the following consequence of the continuation theorem for
implicit functions defined by an entire relation.

\begin{lemma}[Julia]\label{lem:continuation}
...
Julia \cite[Section III]{Julia} proves that this projection cannot
omit a continuum. The stronger Iversen continuation property, due to
Sto\"ilow, is stated in \cite{Eremenko}: the germ can be continued
along an arbitrarily small perturbation of any prescribed path
starting at \(a\). Either statement gives
Lemma~\ref{lem:continuation}.

The second fact concerns growth in a direct tract. In the formulation
of Bergweiler, Rippon, and Stallard \cite[Definition 2.1]{BRS}, an
unbounded domain \(D\subset\C\), with unbounded complement and
piecewise smooth boundary, is a \emph{direct tract} of \(Q\) if
\(Q\) is holomorphic in \(D\), continuous on its closure, and
\[
 |Q|=a\quad\text{on }\partial D,
 \qquad |Q|>a\quad\text{in }D
\]
for some \(a>0\). No condition is imposed on \(Q\) outside
\(\overline D\).
...
The limit in \eqref{eq:tract-growth} has no exceptional radii;
see \cite[Section~9]{BRS} for a proof.
...
We also require \(\delta\) to differ from the modulus of every
finite, nonzero critical value of \(B-m\).
This excludes at most countably many choices.
The level set \(\{|B-m|=\delta\}\) therefore consists of smooth
curves that are locally finite in the plane.
...
\begin{proof}
Start with a regular implicit germ on \(X\) at which \(F_w\ne0\).
Its analytic continuations remain in \(X\).
Lemma~\ref{lem:continuation} gives a point \((z_0,w_0)\in X\) with
\[
...
at \((z_0,w_0)\). Let \(Y\) be the irreducible component containing
its unique local branch there. Analytic continuation gives \(Y\subset X\).
The projection \(Y\to U\) is still finite and proper, and it is
nonconstant. Its image is both open and closed in \(U\), so it is
surjective. We may therefore choose, for any \(w\in U\), a point
\((z,w)\in Y\) satisfying \eqref{eq:root-bound}.
...
In particular, no such point is a zero or pole of \(B-m\).
...
\begin{proof}
The proper mapping theorem and Lemma~\ref{lem:proper} imply that
\(\Gamma\) is a closed analytic curve; see
\cite[Chapter IV, Section 1, Corollary 2]{Herve} for the finite-fibre
projection theorem.
Let \(T\) consist of the singular points of \(\Gamma\), the images
of singular points of \(\X^*\), and the critical values of \(\Phi\)
on the regular part of \(\X^*\).
These sets are locally finite. For the critical points this follows
by using local parameters on the normalizations of the source
branches: \(\lambda=A(z)\) is nonconstant on each component, so its
derivative is not identically zero. Properness ensures that only
finitely many such branches need be considered over a compact
neighbourhood in the target.

For each irreducible component \(\Gamma_i\) of \(\Gamma\), the
restriction over \(\Gamma_i\setminus T\) is a finite unramified
covering. The number of points in a fibre is consequently locally
constant there. The regular part of an irreducible curve is connected,
and remains connected after deletion of a locally finite set; see
\cite[Chapter IV, Section 3, Corollary 1]{Herve}.
Thus there is an integer \(d_i\) such that
\begin{equation}\label{eq:degree}
 \#\Phi^{-1}(s,\lambda)=d_i
 \qquad\bigl((s,\lambda)\in\Gamma_i\setminus T\bigr).
\end{equation}
The integer \(d_i\) counts all preimages, including those belonging
to different components of \(\X^*\).
...
Every irreducible component of \(\X^*\) is obtained from one of
\(\X\) by deleting a discrete set.
Its image under the finite proper map \(\Phi\) is an
...
\]
The function \(b\) is convex and nondecreasing.
Since \(h\) is nonconstant, the maximum principle gives
\[
 b(x)>\log|h(0)|=\lim_{t\to-\infty}b(t)
 \qquad(x\in\R).
\]
If \(b'(x)=0\) at a differentiability point, convexity and
monotonicity would force \(b\) to be constant on \(( -\infty,x]\),
a contradiction. Hence \(\lambda=m+b'(x)>m\).
...
The locally finite set \(T\) is countable, because \(\B\) is second
countable. Define
\[
 E=\{e^x:x\in N\}\ \cup\
   \{\sqrt{s}:(s,\lambda)\in T,\ s\in(0,\infty)\}.
\]
...
\]
For \(R>1/2\), the maximum is attained precisely when
\(\cos(k\theta)=1/(2R)\), giving \(2k\) distinct points.
Thus the bound in Theorem~\ref{thm:main} is sharp for every \(k\).
\end{remark}

\section*{AI tool disclosure}

AI tools were used substantially in developing this work.
Earlier AI-assisted explorations supplied the logarithmic-derivative
formulation used in Section~\ref{sec:products} and the example in
Remark~\ref{rem:sharp}.
OpenAI's GPT-6, accessed through Codex, developed the argument in
Section~\ref{sec:products} combining implicit-function continuation
with growth in a direct tract, the finite-map bound in
Section~\ref{sec:finite}, and their application to maximum modulus
points in Section~\ref{sec:maxima}.
GPT-6 was also used for literature retrieval, proof checking, and
preparation of the English text and \LaTeX{} source.
Responsibility for the mathematical content and the accuracy of the
references rests with the author.
...
\bibitem{Herve}
M.~Herv\'e,
\emph{Several Complex Variables: Local Theory},
Tata Institute of Fundamental Research Studies in Mathematics, vol.~1,
Oxford University Press, London, 1963.
\href{https://mathweb.tifr.res.in/Documents/Publications/Studies/SM_01-Several_Complex_Variables.pdf}
{TIFR digital edition}.
...
\bibitem{Julia}
G.~Julia,
\href{https://www.numdam.org/article/BSMF_1926__54__26_0.pdf}
{Sur le domaine d'existence d'une fonction implicite d\'efinie par
une relation enti\`ere \(G(x,y)=0\)},
