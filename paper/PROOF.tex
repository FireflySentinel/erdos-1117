...
unchanged and gives the normalization \(1+az^k+O(z^{k+1})\) used by
Hayman \cite{Hayman} and Evdoridou, Pardo-Sim\'on, and Sixsmith
\cite{EPS}. Hayman's local curve bound is recalled in
\cite[Theorem A]{EPS}; their Theorem~1.4 describes at most \(k\)
curves, each meeting every sufficiently small circle exactly once.
