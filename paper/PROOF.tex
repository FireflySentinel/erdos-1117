...
\(r>0\).\footnote{If \(|f|\) were constant on \(|z|=r\), the identity
 theorem applied to \(f(z)f^\#(r^2/z)\), where
 \(f^\#(z)=\overline{f(\overline z)}\), would force \(f\) to be a
 monomial. Otherwise \(\theta\mapsto|f(re^{i\theta})|^2\) is nonconstant
 and real analytic on the compact circle, so its maximum set is finite.}
...
Theorem~\ref{thm:main} instead gives a bound at arbitrary radii outside
a countable exceptional set.
...
This follows from the Iversen property stated in \cite{Eremenko}:
the germ continues along an arbitrarily small perturbation of any
path from \(a\).
...
There is a constant \(C\) such that all of them satisfy
...
Analytic continuation of this germ remains in \(X\).
...
\[
 ...
 0<|A(z_0)-m|<\delta.
\]
Then \(w_0\) is not a pole of \(B\), since \(q(z_0)\ne0\),
\(F(z_0,w_0)=0\), and \(p^\#,q^\#\) have no common zero.
...
We claim that \(\C\setminus U\) is unbounded. If \(f\) has a zero
other than the origin, choose arbitrarily large \(R\) such that
\(|z|=R\) contains no zero of \(f\). By the argument principle and
\(B=A^\#\),
\[
 \frac1{2\pi}\int_0^{2\pi}\bigl(B(Re^{i\theta})-m\bigr)\,d\theta
 =n(R,f)-m\ge1,
\]
where \(n(R,f)\) counts the zeros in \(|z|<R\) with multiplicity.
Since \(\delta<1\), the circle cannot lie in \(\{|B-m|<\delta\}\).
If \(f\) has no zero other than possibly the origin, then \(B-m\)
is a nonconstant entire function, whose maximum modulus exceeds
\(\delta\) on arbitrarily large circles. Thus \(\C\setminus U\)
is unbounded.
...
Lemma~\ref{lem:small-products} and the covering argument of
Proposition~\ref{prop:degree} are not formalized.
Given a nonzero, non-monomial entire \(f\), the final Lean theorem
derives \eqref{eq:main-bound} assuming only the fibre bound of
Proposition~\ref{prop:degree} for \(s=r^2>0\), real \(\lambda>m\),
and all \(r\) outside a countable set (\texttt{GlobalFibreBound}).
