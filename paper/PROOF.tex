...
\hypersetup{
  pdftitle={A negative answer to a question of Erdős on maximum modulus points},
  pdfauthor={Qiyuan Gu}
}
...
\title{A negative answer to a question of Erd\H{o}s\\on maximum modulus points}
\author{Qiyuan Gu\\
...
\(r>0\).\footnote{If \(|f|=c>0\) on an arc of \(|z|=r\), the identity
 theorem gives \(f(z)f^\#(r^2/z)=c^2\) on \(\C^*\), where
 \(f^\#(z)=\overline{f(\overline z)}\). Writing \(f=z^m h\),
 \(h(0)\ne0\), and letting \(z\to\infty\) gives
 \(f(z)=O(|z|^m)\). Thus \(f\) is a polynomial of degree at most
 \(m\), hence a monomial. For non-monomial \(f\), the real-analytic
 function \(\theta\mapsto|f(re^{i\theta})|^2\) has a discrete maximum
 set on the compact circle, hence finitely many maximum points.}
Erd\H{o}s asked whether a single such function can satisfy
\[
 \limsup_{r\to\infty}\nu_f(r)=\infty,
...
see also the account in \cite{GP}.
...
There is also a classical local theory near the origin. Dividing \(f\)
in \eqref{eq:taylor} by \(c_0z^m\) leaves its maximum modulus points
unchanged and gives the normalization \(1+az^k+O(z^{k+1})\).
Hayman \cite[Theorem I(iii), pp.~142--143]{Hayman} proved that its
maximum modulus set near zero consists of at most \(k\) analytic curves.
Evdoridou, Pardo-Sim\'on, and Sixsmith \cite[Theorem 1.4(b)]{EPS}
show that each curve meets every sufficiently small circle exactly once.
It follows that \(\nu_f(r)\le k\) for all sufficiently small \(r>0\).
Theorem~\ref{thm:main} is a global counterpart of this local bound:
the same \(k\) controls the count at all but countably many radii.
The unbounded upper limits in \cite{HP,PS} occur on the exceptional radii.
...
\begin{lemma}[Sto\"ilow \cite{Stoilow}; see also \cite{Eremenko}]\label{lem:continuation}
...
The continued graph stays in \(X\) by the identity theorem on its
connected Riemann surface of analytic continuation.
...
\(F(z_0,w_0)=0\) and the absence of common zeros of \(p^\#,q^\#\)
exclude a pole at \(w_0\).
...
\(\chi'(z_0)\ne0\), we have \(A'(z_0)\ne0\). Thus the curve
\eqref{eq:root-relation} is smooth at \((z_0,w_0)\). Let \(Y\) be its
irreducible component through this point. Near \((z_0,w_0)\), it is the same branch as \(X\), hence
...
We claim that \(\C\setminus U\) is unbounded. Otherwise \(U\)
contains a neighbourhood of infinity, on which \(|B-m|<\delta<1\).
Thus \(B\) is meromorphic on \(\C\) and holomorphic and bounded near
infinity, hence rational; so is \(A=B^\#\).
Since \(f'/f=A/z\) is rational, factoring out the finitely many zeros
of \(f\) gives \(f=Pe^R\) for polynomials \(P,R\). Therefore
\(A=zP'/P+zR'\). Boundedness at infinity forces \(R\) to be constant,
so \(A\to\deg P\); the bound \(|A-m|<\delta<1\) gives \(\deg P=m\).
Since \(z^m\) divides \(P\), \(f\) is a monomial, a contradiction.
...
discrete: the absence of horizontal or vertical components ensures that
none is contained in \(zw=0\), \(A(z)=m\), or the pole loci.
Thus every irreducible component of \(\X^*\) is obtained
...
\cite[Chapter II, Theorem 8.8, p.~118]{Demailly} and Lemma~\ref{lem:proper},
...
component \cite[Chapter II, Theorem~7.12, pp.~114--115]{Demailly}.
The map is nonconstant on every source component, so its
...
\]
is proper and, off \(T\), a local biholomorphism, hence a finite covering.
Points of \(\Gamma_i\setminus T\) lie on no other component of
\(\Gamma\), since intersections of components are singular.
The normalization of \(\Gamma_i\) is a connected Riemann surface,
and deleting a locally finite set leaves it connected.
...
For every maximum modulus point \(z\) on \(|z|=r\), reality of
\(\lambda\) gives \(B(\overline z)=\overline{A(z)}=\lambda\).
Since \(\lambda\ne m\) and \(r^2\ne0\), we obtain
...
The arguments of Section~\ref{sec:maxima}, the local
fibre count \eqref{eq:small-fibre}, and Remark~\ref{rem:sharp} are
formalized in Lean 4 with Mathlib at
\url{https://github.com/FireflySentinel/erdos-1117}.
The properness assertion of Lemma~\ref{lem:proper} is formalized for a
supplied entire numerator-denominator representation; finiteness of
the fibres with \(\lambda\notin\{0,m\}\) is proved directly for \(f\).
The propagation of the total fibre bound in Proposition~\ref{prop:degree}
is also formalized. Lemma~\ref{lem:small-products} and the
analytic-curve arguments establishing the regular component properties
remain outside the formalization. The final Lean theorem is conditional
on those properties, or on their consequence \texttt{GlobalFibreBound}.

\section*{Declaration of generative AI use}

GPT-6 Astra was used to propose the mathematical proofs and generate
the Lean formalization. GPT-5.6 Sol and Claude Opus 5 were used for
editorial review of the exposition. The author finished the final
...
\href{https://doi.org/10.1007/BF02790086}{doi:10.1007/BF02790086}.
...
\bibitem{Stoilow}
S.~Sto\"ilow,
Sur les fonctions analytiques dont les surfaces de Riemann ont des
fronti\`eres totalement discontinues,
\emph{Mathematica (Cluj)} \textbf{12} (1936), 123--138.
